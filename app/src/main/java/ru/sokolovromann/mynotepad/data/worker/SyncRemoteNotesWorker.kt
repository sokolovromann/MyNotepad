...
class SyncRemoteNotesWorker(
    ...
    interface SyncRemoteNotesWorkerEntryPoint {
    ...
    private val hiltEntryPoint = EntryPointAccessors.fromApplication(
        applicationContext,
        SyncRemoteNotesWorkerEntryPoint::class.java
    )
    ...
    private fun syncRemoteNotes(owner: String, tokenId: String) {
        runBlocking(dispatcher) {
            noteRepository.syncRemoteNotesOrThrow(owner, tokenId)
        }
    }
}